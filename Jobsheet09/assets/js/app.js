...
// ===== Konfirmasi Form Hapus (Jobsheet 9 - POST) =====
function initHapusConfirm() {
    const formsHapus = document.querySelectorAll(".form-hapus");

    formsHapus.forEach(function (form) {
        form.addEventListener("submit", function (e) {
            const row = form.closest("tr");
            const nama = row ? row.querySelector("td:nth-child(2)")?.textContent : "data ini";
            
            const yakin = confirm('Apakah Anda yakin ingin menghapus "' + nama.trim() + '"?');
            if (!yakin) {
                e.preventDefault(); // Batalkan pengiriman form ke hapus.php
            }
        });
    });
}
...
function initValidasiForm() {
    const form = document.querySelector("form:not(.form-hapus)");
    if (!form) return;

    form.addEventListener("submit", function (e) {
        let valid = true;

        const requiredFields = ["judul", "nama", "pengarang", "email", "telepon", "alamat"];
